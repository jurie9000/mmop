...
class GameServerProtocol(WebSocketServerProtocol):
    ...
    def LOGIN(self, sender: 'GameServerProtocol', p: packet.Packet):
        if p.action == packet.Action.Login:
            ...
            if models.User.objects.filter(username=username, password=password).exists():
                ...
                self.broadcast(packet.ModleDeltaPacket(models.create_dict(self._actor)))
    ...
    def PLAY(self, sender: 'GameServerProtocol', p: packet.Packet):
        ...
        elif p.action == packet.Action.ModleDelta:
            self.send_client(p)

            if sender not in self._known_others:
                sender.onPacket(self, packet.ModleDeltaPacket(models.create_dict(self._actor)))
    ...
    def tick(self):
        ...
        # To do when there are no packets to process
            ...
            if self._update_position():
                actor_dict_afyer: dict = models.create_dict(self._actor)
                self.broadcast(packet.ModleDeltaPacket(models.get_delta_dict(actor_dict_before, actor_dict_afyer)))
